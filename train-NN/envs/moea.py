...
def rand_to_best2(population, samples, scale, best, i):  # DE/rand-to-best/2
    ...
    return (population[r0] + scale * (
                population[best] - population[r0] + population[r1] - population[r2] + population[r3] - population[r4]))
...
def Weighted_Offspring1(popsize, n_ops, gen_window, Off_met, max_gen):
    ...
    for i in range(n_ops):
        ...
        for j in range(len(gen_window) - 1, len(gen_window) - max_gen - 1, -1):
            ...
            if np.any(gen_window[j, :, 0] == i):
                total_success, total_unsuccess = count_success(popsize, gen_window, i, j, Off_met)
                state_value[i] += np.sum(
                    gen_window[j, np.where((gen_window[j, :, 0] == i) & (gen_window[j, :, Off_met] != -1)), Off_met])
                appl += total_success + total_unsuccess
        if appl != 0:
...
def Weighted_Offspring2(popsize, n_ops, window, Off_met, max_gen):
    ...
    for i in range(n_ops):
        if np.sum((window[:, 0] == i) & (window[:, Off_met] != -1)) > 0:
            state_value[i] = np.sum(
                window[np.where((window[:, 0] == i) & (window[:, Off_met] != -1)), Off_met]) / np.sum(
                (window[:, 0] == i) & (window[:, Off_met] != -1));
    if np.sum(state_value) != 0:
        state_value = state_value / np.sum(state_value)
    return state_value
...
def Best_Offspring1(popsize, n_ops, gen_window, Off_met, max_gen):
    ...
    for i in range(n_ops):
        ...
        if np.any((gen_window[len(gen_window) - 1, :, 0] == i) & (gen_window[len(gen_window) - 1, :, Off_met] != -1)):
            ...
            best_t[i] = np.max(gen_window[len(gen_window) - 1, np.where((gen_window[len(gen_window) - 1, :, 0] == i) & (
                        gen_window[len(gen_window) - 1, :, Off_met] != -1)), Off_met])
        # Calculating best in last generation
        if len(gen_window) >= 2 and np.any(
                (gen_window[len(gen_window) - 2, :, 0] == i) & (gen_window[len(gen_window) - 2, :, Off_met] != -1)):
            ...
            best_t_1[i] = np.max(gen_window[len(gen_window) - 2, np.where(
                (gen_window[len(gen_window) - 2, :, 0] == i) & (
                            gen_window[len(gen_window) - 2, :, Off_met] != -1)), Off_met])
        if best_t_1[i] != 0 and np.fabs(n_applications[0] - n_applications[1]) != 0:
            state_value[i] = np.fabs(best_t[i] - best_t_1[i]) / (
                        (best_t_1[i]) * (np.fabs(n_applications[0] - n_applications[1])))
    ...
    for i in range(n_ops):
        ...
        for j in range(len(gen_window) - 1, len(gen_window) - max_gen - 1, -1):
            if np.any((gen_window[j, :, 0] == i) & (gen_window[j, :, Off_met] != -1)):
                gen_best.append(np.max(np.hstack(
                    gen_window[j, np.where((gen_window[j, :, 0] == i) & (gen_window[j, :, Off_met] != -1)), Off_met])))
...
class MOEAEnv(gym.Env):
    ...
    def step(self, action):
        ...
        ob[6:12] = distance.cdist(self.population[[self.r[0], self.r[1], self.r[2], self.r[3], self.r[4], self.best]],
                                  np.expand_dims(self.population[self.i], axis=0)).T / self.max_dist
        ob[12:18] = np.fabs(
            self.copy_F[[self.r[0], self.r[1], self.r[2], self.r[3], self.r[4], self.best]] - self.copy_F[self.i]) / (
                                self.worst_so_far - self.best_so_far)
    ...
    def reset(self):
        ...
        self.lbounds = self.fun.min_bounds
        self.lbounds = np.array(self.lbounds)
        print(self.lbounds)
        self.ubounds = self.fun.max_bounds
        ...
        print("Function info: fun= ", self.func_select[self.fun_index], " with dim = ", self.dim, " with best value= ",
              self.best_value)
        self.budget = 1e4
        self.max_budget = 1e4
        self.generation = 0
